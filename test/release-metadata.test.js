...
test("release versions require an exact three-component tag", () => {
  assert.equal(resolveReleaseVersion("v0.1.5"), "v0.1.5");
  assert.equal(resolveReleaseVersion("V2.10.3"), "v2.10.3");
  assert.equal(resolveReleaseVersion("v0.1"), "v0.0.0");
  assert.equal(resolveReleaseVersion("v0.1.5-beta"), "v0.0.0");
...
test("release metadata loads relative to the deployment base without cache", async () => {
  const calls = [];
  const metadata = await loadReleaseMetadata("./releases/v0.1.5/", async (...args) => {
    calls.push(args);
    return {
      ok: true,
      json: async () => ({
  ...
  assert.deepEqual(calls, [[
    "./releases/v0.1.5/environment.json",
    { cache: "no-store" },
  ]]);
