...
Active heroes SHALL visibly move along the normalized two-dimensional direction from their current position toward their remembered enemy, using their existing profile speed and movement scale. Movement SHALL preserve fixed depth, physics collision behavior, and the centered 9:16 frame on desktop and portrait mobile browsers.

#### Scenario: Hero moves toward remembered enemy
- **WHEN** a started battle contains a hero and an active remembered enemy at a different position
...
- **AND** target selection and movement continue without losing active target memory or producing a runtime error
