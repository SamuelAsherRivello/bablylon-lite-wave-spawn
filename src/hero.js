...
export class Hero {
  constructor(
    ...
    // Keep the body planar: X/Y rotation is locked, while Z rotation remains free.
    this.physics.body.setMassProperties({
      mass: 1,
      inertia: new Vector3(0, 0, 1),
    });
    ...
    // restriction after each physics step so collisions cannot move or tilt the
    // hero out of the XY plane.
    this.physicsObserver = scene.onAfterPhysicsObservable.add(() => {
      if (this.physics?.body) {
        ...
        const angularVelocity = this.physics.body.getAngularVelocity();
        this.physics.body.setAngularVelocity(
          new Vector3(0, 0, angularVelocity.z),
        );
  ...
  updateDepthSort() {
    // Heroes are strictly 2D physics bodies, so depth never changes with Y.
  }
}
